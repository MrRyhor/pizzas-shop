...
class DbOperations {
    ...
    //             // в нашем случае if(item.cart) если есть поле cart - массив товаров в корзине для юзера с id, то в массив arrayProperty закидываем value
    //             if (item.cart) {
    //                 const findObj = item.cart.find(el => el.id === value.id)
    //                 if (findObj) {
    //                     findObj.count++
